...
def update_priordict_standard_mpta():
    ...
    prior.priordict_standard.update({
        ...
        '(.*_)?chrom_fix_gp_log10_A':   [-18, -11],
        '(.*_)?chrom_fix_gp_gamma':     [0, 7],
...
def make_psr_gps_fourier(psr, max_cadence_days=14, Tspan=None, GlobalTspan = None,background=True, bkgrnd_fixed=False, bkgrnd_fixed_log10A=jnp.log10(2e-15), bkgrnd_fixed_gamma=13/3, curn=False, curn_fix_gamma=False, red=True, dm=True, chrom=True, chrom_fix=False, sw=True, dm_sw_free=False, band=False, band_low=False, band_alpha=False):
    ...
    return (([signals.makegp_fourier(psr, signals.powerlaw_gwb, components=psr_components, name='bkgrnd')] if background and not bkgrnd_fixed and not curn else []) + \
            ...
            ([signals.makegp_fourier(psr, signals.powerlaw, components=psr_components, fourierbasis=signals.fourierbasis_chrom, name='chrom_gp')] if chrom and not chrom_fix else [])+ \
            ([signals.makegp_fourier(psr, signals.powerlaw, components=psr_components, fourierbasis=signals.fourierbasis_chrom_fix, name='chrom_fix_gp')] if chrom_fix and not chrom else [])+ \
...
def make_psr_gps_fftint(psr, max_cadence_days=14, Tspan=None, GlobalTspan = None , background=True, bkgrnd_fixed=False, bkgrnd_fixed_log10A=jnp.log10(2e-15), bkgrnd_fixed_gamma=13/3, curn=False, curn_fix_gamma=False, red=True, dm=True, chrom=True, chrom_fix=False, sw=True, dm_sw_free=False, band=False, band_low=False, band_alpha=False):
    ...
    return (([signals.makegp_fftcov(psr, signals.powerlaw_gwb, components=psr_knots, name='bkgrnd')] if background  and not bkgrnd_fixed and not curn else []) + \
            ...
            ([signals.makegp_fftcov_chrom(psr, signals.powerlaw, components=psr_knots, name='chrom_gp')] if chrom and not chrom_fix else [])+ \
            ([signals.makegp_fftcov_chrom_fix(psr, signals.powerlaw, components=psr_knots, name='chrom_fix_gp')] if chrom_fix and not chrom else [])+ \
...
def single_pulsar_noise(psr, fftint=True, max_cadence_days=14, Tspan=None, GlobalTspan = None , noisedict={}, tm_variable=False, timing_inds=None, outliers=False, tnequad=True, ecorr=True, global_ecorr=False,
                        background=False, bkgrnd_fixed=False, bkgrnd_fixed_log10A=jnp.log10(2e-15), bkgrnd_fixed_gamma=13/3, curn=False, curn_fix_gamma=False, red=True, dm=True, chrom=True, sw=True, dm_sw_free=False, band=False, band_low=False, band_alpha=False, # GP models
